...
void readSudoku(string fname){
	...
	//numVariables = (size*size)*size;
	//cout << numVariables << " " << size <<  endl;
}
...
void exec_fn(void){
	//inserting "p cnf" "number of variables" "number of variables" at the beginning of cnf file 
	ifstream cnf("cnf.txt");
	ofstream cnf_temp("cnf_temp.txt");
	string tempStr ;
	cnf_temp << "p cnf " << numVariables << " " << clausesCount << "\n";
	cnf_temp << cnf.rdbuf() ;
	cnf.close();
	cnf_temp.close();
	remove("cnf.txt");
	rename("cnf_temp.txt","cnf.txt");
...
int main(int argc,char **argv){
	...
	string ip_f_name =  argv[1];
	readSudoku(ip_f_name);
